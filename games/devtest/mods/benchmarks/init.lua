...
minetest.register_chatcommand("bench_bulk_get_node_raw", {
	params = "",
	...
	func = function(name, param)
		...
		local function bench()
			...
			for i=1,#pos_list do
				...
				local nid = minetest.get_node_raw(pos_i.x, pos_i.y, pos_i.z)
				-- Make sure the name lookup is never optimized away.
				-- Table allocation might still be omitted. But only accessing
				-- the name of a node is a common pattern anyways.
				if nid == -42 then
		...
		minetest.chat_send_player(name, "Benchmarking minetest.get_node_raw. Warming up ...")
		bench()

		minetest.chat_send_player(name, "Warming up finished, now benchmarking ...")
		local result_us = bench()

		local msg = string.format("Benchmark results: minetest.get_node_raw loop 1: %.2f ms",
				result_us / 1000)
		return true, msg
	end,
})

minetest.register_chatcommand("bench_bulk_get_node_raw_lookup", {
	params = "",
	...
	func = function(name, param)
		...
		local function bench()
			...
			for i=1,#pos_list do
				local pos_i = pos_list[i]
				local nid = minetest.get_node_raw(pos_i.x, pos_i.y, pos_i.z)
				local name = minetest.get_name_from_content_id(nid)
				-- Make sure the name lookup is never optimized away.
				-- Table allocation might still be omitted. But only accessing
				-- the name of a node is a common pattern anyways.
		...
		minetest.chat_send_player(name, "Benchmarking minetest.get_node_raw+get_name_from_content_id. Warming up ...")
		bench()

		minetest.chat_send_player(name, "Warming up finished, now benchmarking ...")
		local result_us = bench()

		local msg = string.format("Benchmark results: minetest.get_node_raw+get_name_from_content_id loop 1: %.2f ms",
				result_us / 1000)
		return true, msg
	end,
})

minetest.register_chatcommand("bench_bulk_get_node_vmanip", {
	params = "",
	...
	func = function(name, param)
		...
		local function bench()
			local start_time = minetest.get_us_time()
			local vm = minetest.get_voxel_manip(pos:offset(1,1,1), pos:offset(100,100,100))
			local data = vm:get_data()
			local mid_time = minetest.get_us_time()
			for i=1,99*99*99 do
				local nid = data[i]
				-- Make sure the name lookup is never optimized away.
				-- Table allocation might still be omitted. But only accessing
			...
			return minetest.get_us_time() - start_time, mid_time - start_time
		end

		minetest.chat_send_player(name, "Benchmarking minetest.get_voxel_vmanip+get_data+loop . Warming up ...")
		bench()

		minetest.chat_send_player(name, "Warming up finished, now benchmarking ...")
		local result_us, get_data_us = bench()

		local msg = string.format("Benchmark results: minetest.get_voxel_vmanip+get_data+loop loop 1: %.2f ms of which get_data() %.2f ms",
